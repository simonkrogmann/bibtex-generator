...
def add_note_to_bibtex(bibtex: str, note: str) -> str | bool:
    obj = bibtexparser.loads(bibtex)
    if len(obj.entries) != 1:
        return False
    if 'note' in obj.entries[0]:
        obj.entries[0]['note'] += " " + note
    else:
        obj.entries[0]['note'] = note
    return bibtexparser.dumps(obj)
    ...
    for i, ref in enumerate(filtered):
        ...
        if ref.doi:
            ...
            if not ref.bibtex:
                ...
                continue
            new_bibtex = add_note_to_bibtex(ref.bibtex, 'This entry was generated by AI, please check.')
            if new_bibtex:
                ref.bibtex = new_bibtex
            else:
                print('Warning: Unusual LLM output for', ref.text)
        else:
